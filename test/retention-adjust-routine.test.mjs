...
describe('the two temporary DEV before/after assets', () => {
  ...
  for (const key of ['dental', 'eye']) {
    test(`spp-before-after-${key}.jpg (when supplied) is a real, square, optimized JPEG`, (t) => {
      if (!existsSync(resolve(here, '..', 'assets', `spp-before-after-${key}.jpg`))) return t.skip('not supplied in this release');
...
describe('the recommendation card is fully wired in Liquid', () => {
  ...
  test('the asset base is emitted ONLY when before/after images are approved (release: off, so nothing is requested)', () => {
    assert.match(cancelLiquid, /assign spp_before_after_approved = false/);
    assert.match(screenMarkup('cancel-offer'), /\{%-? if spp_before_after_approved %\} data-spp-asset-base="\{\{ 'spp-cancel-benefits-mobile\.jpg' \| asset_url \}\}"\{% endif %\}/);
  });
});
